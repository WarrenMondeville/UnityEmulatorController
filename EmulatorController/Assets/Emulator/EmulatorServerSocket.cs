...
class EmulatorServerSocket : MonoBehaviour {
	...
	static readonly byte[][] _types = new byte[][]{
		new byte[]{ 0x00, 0x00, 0x00, 0x00 }, // NOTHING        空的                                    
		new byte[]{ 0x08, 0x01, 0x12, 0x15 }, // MOTION         动作
		new byte[]{ 0x08, 0x02, 0x1A, 0x19 }, // GYROSCOPE      陀螺仪 
		new byte[]{ 0x08, 0x03, 0x22, 0x19 }, // ACCELEROMETER  加速器
		new byte[]{ 0x08, 0x04, 0x00, 0x00 }, // DEPTH_MAP - @NotImplemented 深度图，还没有实现
		new byte[]{ 0x08, 0x05, 0x32, 0x1E }, // ORIENTATION    方向
